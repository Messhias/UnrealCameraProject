 ...
 */
UCLASS(abstract)
class ACombatEnemy : public ACharacter, public ICombatAttacker, public ICombatDamageable, public ILockOnTarget
{
	GENERATED_BODY()
...
public:
	...
	// ~begin ILockOnTarget interface

	/** Returns the world location where the camera should lock on (center of torso) */
	virtual FVector GetLockOnLocation() const override;

	/** Returns true if this enemy is alive and valid for lock-on */
	virtual bool IsLockOnValid() const override;

	/** Returns the priority for lock-on selection */
	virtual int32 GetLockOnPriority() const override;

	// ~end ILockOnTarget interface
