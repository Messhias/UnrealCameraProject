...
FVector ACombatEnemy::GetLockOnLocation() const
{
	// Return the center of the character's capsule (torso level)
	// This provides a good lock-on point that's roughly at the center of the enemy
	FVector Location = GetActorLocation();
	
	// Add half the capsule height to get to the center of the capsule
...
bool ACombatEnemy::IsLockOnValid() const
{
	// Enemy is valid for lock-on if it's alive
	return CurrentHP > 0.0f;
}

int32 ACombatEnemy::GetLockOnPriority() const
{
	// Default priority - can be overridden in derived classes if needed
	return 0;
}
